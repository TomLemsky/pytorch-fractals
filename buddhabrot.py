...
def buddhabrot(center, diameter, resolution, iterations=20):
    ...
    complex_float_type = torch.complex64
    ...
    # randomize within box around the pixel with width pixel_width
...
counts  = torch.zeros(resolution**2)
resolution_p = resolution+2
for i in range(1,samples_per_pixel):
    print(f"{i}-th round of samples")
    counts += buddhabrot(center=center, diameter=diameter, resolution=resolution_p,iterations=iterations)

    #largest values dominated by upper left corner etc. because that's what

    image = counts.reshape(resolution_p,resolution_p)
    # desregard corners, because that is where all the out-of-bounds values end up
    image = image[1:-1,1:-1]
    maximum_pixel = image.quantile(0.99)
    #image = image/image.max()
    save_image(image/maximum_pixel, f"buddhabrot.png")
